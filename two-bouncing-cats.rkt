...
(define (move-cat-up w) 
  (if (cat-is-selected? (world-cat1 w))
      (direction-up-cat (world-cat1 w)) 
      (if (cat-is-selected? (world-cat2 w))
          (direction-up-cat  (world-cat2 w)) 
          w)
      ))

(define (move-cat-down w) 
  (if (cat-is-selected? (world-cat1 w))
      (direction-down-cat (world-cat1 w)) 
   (if (cat-is-selected? (world-cat2 w))
      (direction-down-cat  (world-cat2 w))
      w)
   )) 
(define (move-cat-left w)
   (if (cat-is-selected? (world-cat1 w))
      (direction-left-cat (world-cat1 w) ) 
   (if (cat-is-selected? (world-cat2 w))
      (direction-left-cat  (world-cat2 w) )
      w)))
  
(define (move-cat-right w)
   (if (cat-is-selected? (world-cat1 w))
      (direction-right-cat (world-cat1 w)) 
   (if (cat-is-selected? (world-cat2 w))
      (direction-right-cat  (world-cat2 w) )
      w)))
(define (cat-is-selected? cat )
        (cat-selected? cat))

(define (direction-down-cat cat )
  (make-world (make-cat (cat-x-pos cat)(cat-y-pos cat)
                                        (cat-selected? cat) "south");;make helper
              (world-cat2 w) (world-paused? w)))
  

(define (direction-right-cat cat )
  (make-world (make-cat((cat-x-pos cat)(cat-y-pos cat)
                                        (cat-selected? cat) "east"));;make helper
              (world-cat2 w) (world-paused? w))
  )

(define (direction-left-cat cat )
  (make-world (make-cat((cat-x-pos cat)(cat-y-pos cat)
                                        (cat-selected? cat) "west"));;make helper
              (world-cat2 w) (world-paused? w))
  )
(define (direction-up-cat cat )
  (make-world (make-cat((cat-x-pos cat)(cat-y-pos cat)
                                        (cat-selected? cat) "north"));;make helper
              (world-cat2 w) (world-paused? w))
  )
